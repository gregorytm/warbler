...
app.configt['WTF_CSRF_ENABLED'] = False
...
class MessageViewTestCase(TestCase):
    ...
    def test_users_index(self):
        with self.client as c:
            ...
            self.assertIn("@jabc", str(resp.data))
            self.assertIn("@whogives", str(resp.data))
            self.assertIn("#ajdkf", str(resp.data))
    ...
    def test_users_search(self):
        with self.client as c:
            ...
            self.assertIn("testuser", str(resp.data))
            self.assertIn("@jtealkjt", str(resp.data))

            self.assertNotIn("@jabc", str(resp.data))
            self.assertNotIn("@whogives", str(resp.data))
            self.assertNotIn("@ajdkf", str(resp.data))
    ...
    def setup_likes(self):
        m1 = Message(text="trending warbler", user_id=self.testuser_id)
        m2 = Message(text="Eathing some food", user_id=self.testuser_id)
        m3 = Message(id=9876, text="omgomg", user_id=self.u1_id)
    ...
    def test_add_like(self):
        ...
        with self.client as c:
            ...
            self.assertEual(likes[0].user_id, self.testuser_id)
    ...
    def test_unauthenticated_like(self):
        ...
        with self.client as c:
            resp = c.post(f"/messages/{mid}/like", follow_redirects=True)
            self.assertEqual(resp.status_code, 200)
    ...
    def setup_followers(self):
        ...
        f3 = Follows(user_being_followed_id=self.testuser_id, user_following_id=self.testuser_id)
